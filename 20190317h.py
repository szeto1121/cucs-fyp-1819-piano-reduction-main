...
for hidden in [50, 100, 150, 200, 250, 300]:
    ...
    models = {}
    for i in train_list:
        models['GRU_2 (%d) %d' % (hidden, i)] = bidirectional_gru(features, LENGTH, hidden)
        models['GRU_1 (%d) %d' % (hidden, i)] = km.gru_with_features(features, hidden)
    progress = ''
