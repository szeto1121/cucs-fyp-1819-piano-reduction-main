...
def bidirectional_gru(features, length=20):
...
for LENGTH in [1, 2, 3, 4, 8, 12, 16, 20, 24]:
    ...
    models = {}
    for i in train_list:
        models['GRU_2 (%d) %d' % (LENGTH, i)] = bidirectional_gru(features, LENGTH)
        models['GRU_1 (%d) %d' % (LENGTH, i)] = km.gru_with_features(features)
    progress = ''
