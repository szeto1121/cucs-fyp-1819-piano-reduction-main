...
models = {}
for i in train_list:
    models['GRU_2 -new %d' % i] = bidirectional_gru(features)
    models['GRU_1 -new %d' % i] = km.gru_with_features(features)
progress = ''

for i in train_list:
    data = ScoreData.load('score_data/%d.pkl' % i)
    
    training_data, testing_data = data.split_in(0.8, 0.9)
...
models = {}
for i in train_list:
    models['GRU_2 -chord %d' % i] = bidirectional_gru(features)
    models['GRU_1 -chord %d' % i] = km.gru_with_features(features)
progress = ''
