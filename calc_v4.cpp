...
int main(int argc, char* argv[]){
    ...
    set<pair<pair<int, int>, int> > failed;
	...
	while (con < 10000){
        ...
        for (int i = 2; i <= n; i++)
            for (int j = 1; j <= 128; j++)
                for (int l = 0; l <= 1; l++)
                    if (failed.find(make_pair(make_pair(i, j), l)) == failed.end() && moment[i][j] > 0)
                        possible.push_back(make_pair(make_pair(i, j), l));
        if (possible.size() == 0)
            break;
        //printf("%d: %d\n", k, possible.size());
        int rpos = (rand() % 32768 * 32768 + rand()) % possible.size();
        r = possible[rpos].first.first;
        c = possible[rpos].first.second;
        choice = possible[rpos].second;
		...
		if (nowpenalty < minpenalty){
			...
			con++;
            failed.insert(make_pair(make_pair(r, c), choice));
        }
	}
