...
int compute_penalty(){
	...
	for (int i=1;i<=n;i++){
		...
		//crowdness ~ busyness 
		if (leftcnt == 4)  tmppenalty[i][3] += 15;
		else if (leftcnt == 5) tmppenalty[i][3] += 30;
		if (rightcnt == 4)  tmppenalty[i][3] += 15;
		else if (rightcnt == 5) tmppenalty[i][3] += 30;
			
		//range
		if ((larr[i][leftcnt] - larr[i][1]) > hand_size) tmppenalty[i][4] += 15;
		else if ((larr[i][leftcnt] - larr[i][1]) > hand_size+2) tmppenalty[i][4] += 30;
		
		if ((rarr[i][rightcnt] - rarr[i][1]) > hand_size) tmppenalty[i][4] += 15;
		else if ((rarr[i][rightcnt] - rarr[i][1]) > hand_size+2) tmppenalty[i][4] += 30;
		...
		if (i>=2){
			...
			//		penalty[i]+=50;
			tmppenalty[i][5] += abs(lhandpos[i]-lhandpos[i-1])*5;
			tmppenalty[i][5] += abs(rhandpos[i]-rhandpos[i-1])*5;
		...
		for (int j = 1; j <= 6; j++){
			if (j<=3){
				tmppenalty[i][0] += tmppenalty[i][j]*w[1];
				sum += tmppenalty[i][j]*w[1];
			}
			else {
				tmppenalty[i][0] += tmppenalty[i][j]*w[j-2];
				sum += tmppenalty[i][j]*w[j-2];
			}
		}
